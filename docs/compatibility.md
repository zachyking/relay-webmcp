...
| WebMCP | Imperative top-level registration, tool annotations, abort cleanup, unsupported fallback, human-page exclusion, and mutation idempotency tests | Passing |
...
6. Revoke the active binding from the human dashboard and confirm the next call is unauthorized.
7. Record client version, transport, auth mode, date, and any deviations in the release evidence.
